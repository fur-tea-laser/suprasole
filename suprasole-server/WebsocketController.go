...
func (This *_WebsocketController_) HandleRequest_GetWebsocketConnection(
	...
	if CONNECTED__Status_WebsocketConnection == status_WebsocketConnection_captured {
		This.TakeoverStatus_WebsocketConnection_state = PENDING__TakeoverStatus_WebsocketConnection
	}
	This.Mutex.Unlock()
	if CONNECTED__Status_WebsocketConnection == status_WebsocketConnection_captured {
	...
	case submissionReply_GetWebsocketConnection := <-submission_GetWebsocketConnection.ReplyChannel:
		if nil == submissionReply_GetWebsocketConnection.Error_Submission_maybe {
			...
			panic("invalid path: _WebsocketController_ HandleRequest_GetWebsocketConnection")
		}
	}
}
...
func (This *_WebsocketController_) CloseWithCode_WebsocketConnection(
	...
	This.Mutex.Lock()
	if CONNECTED__Status_WebsocketConnection == This.Status_WebsocketConnection_state {
...
func (This *_WebsocketController_) WritePayload_BinaryMessage(
	...
	This.Mutex.Lock()
	if CONNECTED__Status_WebsocketConnection == This.Status_WebsocketConnection_state && id_WebsocketConnection_expected == This.Id_WebsocketConnection_state {
		WebsocketConnection_captured = This.WebsocketConnection_state
	} else if This.Status_WebsocketConnection_state != CONNECTED__Status_WebsocketConnection {
		error_state__WebsocketConnection__maybe = ERROR__NOT_CONNECTED___WRITE_PAYLOAD__BINARY_MESSAGE
	} else if This.Id_WebsocketConnection_state != id_WebsocketConnection_expected {
		error_state__WebsocketConnection__maybe = ERROR__CONNECTION_ID_MISALIGNED___WRITE_PAYLOAD__BINARY_MESSAGE
	} else {
		panic("invalid path: _WebsocketController_ WritePayload_BinaryMessage")
	}
	This.Mutex.Unlock()
